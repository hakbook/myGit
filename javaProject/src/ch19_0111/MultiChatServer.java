...
public class MultiChatServer {
	...
	private void serviceStart() {
		...
		try {
			...
			while (true) {
				...
				//클라이언트별로 메시지 수신을 위해 스레드사 필요함
				//10명이 접속하면 10개의 스레드가 생성(속도가 느려짐)
				SreverReceiver receiverThread = new SreverReceiver(socket);
				receiverThread.start();
				System.out.println("현재 실행준인 스레드 네임 : " + receiverThread.getName());
	...
	class SreverReceiver extends Thread {
		Socket socket;
		DataInputStream in;
		DataOutputStream out;
		public SreverReceiver(Socket socket) {
			this.socket = socket;
			try {
